...
pub const LsColors = struct {
    allocator: Allocator,
    copied_str: ?[]const u8,
    entry_type_mapping: std.enums.EnumMap(EntryType, Style),
    ...
    pub fn parseStrOwned(allocator: Allocator, s: []const u8) !Self {
        var entry_types: std.enums.EnumMap(EntryType, Style) = .init(.{});
                ...
                if (iter.next()) |sty| {
                    ...
                    if (std.mem.eql(u8, "ln", pattern) and std.mem.eql(u8, "target", sty)) {
                        ...
                        if (EntryType.fromStr(pattern)) |entry_type| {
                            entry_types.put(entry_type, style_parsed);
                        } else {
...
test "parse geoff.greer.fm default lscolors" {
    ...
    try expectEqual(lsc.entry_type_mapping.get(.Directory).?, expected);
}
